...
简介：在Windows11系统中，如果你正在开发办公自动化脚本，特别是处于开发初期的阶段，可能会遇到需要让脚本频繁请求管理员权限的问题，这是由于Windows在不同软件之间采取权限隔离的政策。如果你希望通过办公自动化脚本（默认以普通用户权限运行）去模拟鼠标键盘操作高权限的办公软件，就必须授予办公自动化脚本足够高的权限。
